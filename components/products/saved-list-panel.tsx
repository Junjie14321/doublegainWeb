            ...
            {savedItems.map((item) => (
              <li key={item.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-ui text-text-primary truncate">{item.name[locale]}</p>
                </div>
                <button
                  onClick={() => toggle(item)}
      ...
      {savedItems.length > 0 && (
        <div className="p-4 border-t border-border-color flex-shrink-0">
          <button
            ...
            className="w-full bg-primary hover:bg-primary-dark text-white font-ui text-xs md:text-sm py-3 px-4 rounded-lg transition-colors uppercase tracking-wide flex items-center justify-center gap-2"
          >
