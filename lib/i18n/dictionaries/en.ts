export const en = {
  metadata: {
    title: 'Master 2 - Premium Asian Kitchen Ingredients',
    description: 'Supplying specialty sauces and noodles to Asian kitchens from caterers to hotels and restaurants since 1996.',
  },
  nav: {
    home: 'Home',
    products: 'Products',
    sauces: 'Sauces',
    noodles: 'Noodles',
    ingredients: 'Pre-made Ingredients',
    contact: 'Contact',
  },
  hero: {
    tagline: 'Supplying specialty sauces and noodles to Asian kitchens from caterers to hotels and restaurants since 1996',
    cta: 'Request Price List',
    secondaryCta: 'View Products',
  },
  trust: {
    title: 'Trusted by Commercial Kitchens',
    years: '28+ Years',
    yearsDesc: 'Industry Experience',
    clients: '500+',
    clientsDesc: 'Commercial Clients',
    products: '50+',
    productsDesc: 'Premium Products',
  },
  value: {
    title: 'Why Commercial Kitchens Choose Us',
    time: {
      title: 'Reduce Prep Time',
      description: 'Pre-made ingredients and ready-to-use sauces cut kitchen preparation by up to 60%.',
    },
    consistency: {
      title: 'Ensure Consistency',
      description: 'Standardized formulations guarantee the same taste across every dish, every time.',
    },
    scale: {
      title: 'Scale with Confidence',
      description: 'Reliable bulk supply and flexible ordering help you grow without supply chain worries.',
    },
  },
  categories: {
    title: 'Our Product Range',
    sauces: {
      name: 'Sauces',
      tagline: 'Consistent depth and flavor in every pour',
    },
    noodles: {
      name: 'Noodles',
      tagline: 'Authentic texture for traditional dishes',
    },
    ingredients: {
      name: 'Pre-made Ingredients',
      tagline: 'Speed and convenience without compromise',
    },
    viewAll: 'View All',
  },
  featured: {
    title: 'Featured Products',
    viewAll: 'View All Products',
  },
  benefits: {
    title: 'From Kitchen Challenge to Solution',
    items: [
      {
        problem: 'Inconsistent sauce quality from batch to batch',
        solution: 'Our sauces are formulated for consistent flavor in every bottle',
      },
      {
        problem: 'Hours spent on ingredient prep daily',
        solution: 'Pre-made ingredients ready to use, saving precious kitchen time',
      },
      {
        problem: 'Supply shortages during peak seasons',
        solution: 'Reliable stock and flexible ordering for uninterrupted service',
      },
    ],
  },
  faq: {
    title: 'Frequently Asked Questions',
    items: [
      {
        question: 'What is your minimum order quantity?',
        answer: 'Our minimum order varies by product category. For sauces, we typically require a minimum of 1 carton (12 bottles). Contact us via WhatsApp for specific product minimums.',
      },
      {
        question: 'Do you offer bulk pricing for restaurants?',
        answer: 'Yes, we offer competitive bulk pricing for all commercial clients including restaurants, hotels, and catering companies. Request our price list via WhatsApp for detailed pricing tiers.',
      },
      {
        question: 'How do I place an order?',
        answer: 'Simply contact us via WhatsApp with your order details. Our team will confirm availability, provide pricing, and arrange delivery to your location.',
      },
      {
        question: 'What is your delivery coverage?',
        answer: 'We deliver to commercial kitchens throughout the region. Delivery schedules and coverage areas can be discussed when you contact us.',
      },
      {
        question: 'Are your products suitable for halal kitchens?',
        answer: 'Please contact us directly to discuss specific product certifications and suitability for your kitchen requirements.',
      },
    ],
  },
  cta: {
    title: 'Ready to Streamline Your Kitchen?',
    description: 'Get our full price list and discover how Master 2 can help your kitchen operate more efficiently.',
    button: 'Contact Us on WhatsApp',
  },
  footer: {
    description: 'Premium supplier of specialty sauces, noodles, and pre-made ingredients for commercial Asian kitchens.',
    quickLinks: 'Quick Links',
    categories: 'Categories',
    contact: 'Contact',
    rights: 'All rights reserved.',
  },
  product: {
    inquire: 'Inquire on WhatsApp',
    viewDetails: 'View Details',
    backToProducts: 'Back to Products',
    relatedProducts: 'Related Products',
    bestSeller: 'Best Seller',
    premium: 'Premium',
  },
  cart: {
    title: 'Inquiry List',
    empty: 'Your inquiry list is empty.',
    emptyHint: 'Add products you\'re interested in, then send a quote or sample request.',
    requestQuote: 'Request Quote via WhatsApp',
    requestSample: 'Request Sample via WhatsApp',
    requestPriceList: 'Request Price List',
    clearAll: 'Clear All',
    addToList: 'Add to List',
    added: 'Added',
    remove: 'Remove',
    items: 'item(s) in list',
  },
  whatsapp: {
    priceListIntent: 'I would like to request your price list.',
    productInquiry: 'I am interested in your',
    businessType: {
      restaurant: 'I run a restaurant',
      hotel: 'I work for a hotel',
      distributor: 'I am a distributor',
      caterer: 'I run a catering company',
    },
  },
}

export type Dictionary = typeof en
